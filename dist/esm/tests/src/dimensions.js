...
describe('dimensions: getWidth() and getHeight()', () => {
    ...
    it(`should measure the document's width and height`, function () {
        ...
        assert.strictEqual(getWidth(document, 'content'), expectedWidth, 'getWidth(document, "content")');
        assert.strictEqual(getHeight(document, 'content'), expectedHeight, 'getHeight(document, "content")');
        assert.strictEqual(getRect([document, 'content']).width, expectedWidth, 'getRect([document, "content"]).width');
        assert.strictEqual(getRect([document, 'content']).height, expectedHeight, 'getRect([document, "content"]).height');
        assert.strictEqual(getWidth(document, 'padding'), expectedWidth, 'getWidth(document, "padding")');
        assert.strictEqual(getHeight(document, 'padding'), expectedHeight, 'getHeight(document, "padding")');
        assert.strictEqual(getRect([document, 'padding']).width, expectedWidth, 'getRect([document, "padding"]).width');
        assert.strictEqual(getRect([document, 'padding']).height, expectedHeight, 'getRect([document, "padding"]).height');
        ...
        assert.strictEqual(getHeight(document, 'scroll'), expectedHeight, 'getHeight(document, "padding")');
        assert.strictEqual(getRect([document, 'scroll']).width, expectedWidth, 'getRect([document, "scroll"]).width');
        assert.strictEqual(getRect([document, 'scroll']).height, expectedHeight, 'getRect([document, "scroll"]).height');
        assert.strictEqual(getWidth(document, 'border'), expectedWidth, 'getWidth(document, "border")');
        assert.strictEqual(getHeight(document, 'border'), expectedHeight, 'getHeight(document, "border")');
        assert.strictEqual(getRect([document, 'border']).width, expectedWidth, 'getRect([document, "border"]).width');
        assert.strictEqual(getRect([document, 'border']).height, expectedHeight, 'getRect([document, "border"]).height');
        ...
        assert.strictEqual(getRect([document, 'margin']).width, expectedWidth, 'getRect([document, "margin"]).width');
        assert.strictEqual(getRect([document, 'margin']).height, expectedHeight, 'getRect([document, "margin"]).height');
        el.remove();
    });
